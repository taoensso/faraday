...
(deftest batch-simple
  (let [i0 {:id 0 :name "foo"}
    ...
    (far/batch-write-item creds {table {:put [i0 i1]}})

    (is (and (= (far/get-item creds table {:id 0}) i0)
             (= (far/get-item creds table {:id 1}) i1))
        "batch-write-item :put works")

    (let [r1 (far/batch-get-item creds
               {table {:prim-kvs    {:id [0 1]}
    ...
    (is (and (not (far/get-item creds table {:id 0}))
             (not (far/get-item creds table {:id 1})))
        "batch-write-item :delete works")))

(deftest serialization
  (let [data (dissoc nippy/stress-data :bytes)]
    (far/put-item creds table {:id 10 :nippy-data (far/freeze data)})
    (is (= (far/get-item creds table {:id 10}) {:id 10 :nippy-data data})
...
(deftest conditional-put
  (let [i0 {:id 0 :name "foo"}
    ...
    (is (thrown? #=(far/ex :conditional-check-failed)
                 (far/put-item creds table i1 {:expected {:id false}}))
        "Put with bad conds throws exceptions")

    (is (and (= (far/put-item creds table i1 {:expected {:id 1}})        nil)
             (= (far/put-item creds table i2 {:expected {:id false}})    nil)
             (= (far/put-item creds table i2 {:expected {:id    2
                                                         :dummy false}}) nil))
